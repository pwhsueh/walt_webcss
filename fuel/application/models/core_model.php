...
public function do_update_fbid2resume($account,$fbid){
 ...
 //print_r($account_arr);
    ...
    public function get_order_info($order_id,$member_id)
    {
        $sql = @"SELECT mo.order_id, mo.product_plan, mo.order_name, mo.order_email, mo.order_mobile, mo.order_addr, mo.order_vat_number, mo.order_invoice_title, mo.RtnCode, 
                        ...
                        mo.account,mo.mailing_date,mo.account_nm,mo.mailing_amount
                 FROM mod_order mo WHERE order_id=? AND member_id=?";
        $para = array($order_id,$member_id);
        $query = $this->db->query($sql, $para);
    ...
    public function get_order_total_amount($order_id)
    {
        $sql = @"SELECT SUM(amount*num) as total_amount
    ...
    public function do_contact($name,$email,$phone,$content)
    {
        $sql = @"INSERT INTO mod_contact (name,email,phone,content,modi_time) VALUES ('$name','$email','$phone','$content',NOW()) ";
        $para = array(
                $name,
                $email,
                $phone,

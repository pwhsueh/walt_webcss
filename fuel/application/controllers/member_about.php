...
class Member_about extends CI_Controller {
	...
	{	   
		...
		$vars['city_result'] = $city_result;	  
		$vars['register_url'] = base_url()."do_register";
		$vars['views'] = 'register'; 
	...
	function do_register()
	{
		...
		$this->load->module_model(MEMBER_FOLDER, 'member_manage_model');
		$member_name 			= $this->input->get_post("member_name");
		$member_email 			= $this->input->get_post("member_account");
		$member_pwd 		    = $this->input->get_post("member_pwd");
	...
	function send_new_pwd()
	{
		...
		if(!empty($email) && is_ajax())
		{
			...
			if($chk_email === TRUE)
			{
				...
				$this->email->from('waltcomt@walt.com.tw', 'Walt - 華特燈飾網站');
				$this->email->to($email); 

				$this->email->subject("Walt - 華特燈飾網站會員密碼通知信");

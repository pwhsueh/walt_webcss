...
<script type="text/javascript">
	...
	$j(document).ready(function($) {
       var config =
	    ...
	    config.imageBrowser_listUrl = "<?php echo $get_photo_data_to_ck_url?>";
		$j( 'textarea#pro_desc' ).ckeditor(config);
		$j( 'textarea#pro_format' ).ckeditor(config);
		$j( 'textarea#pro_ship_note' ).ckeditor(config);
		...
		$j("#add_plan").click(function() {
			...
			$.post(url, tpl_data, function(data, textStatus, xhr) {
				...
				{
					$j('.blockText').find('span').html(data.msg);
					$j('.blockText').fadeIn(500).delay(200).fadeOut(500);						
				}
			});
			...
			<table class="table table-bordered">
				...
				<tbody>
						...
						<td>
							...
							<select name="pro_cate" class="form-control input-sm">
								<?php 
									...
									{
								?>
									<option value="<?php echo $cate_row->id?>" <?php if($cate_row->id==$row->pro_cate) echo "selected"?>><?php echo $cate_row->code_name?></option>
								<?php 
									}
								?>
							</select>
					...
					<tr>
						...
						<td>
							<div class="col-xs-2">
								<input type="text" class="form-control input-sm" name="pro_add_time" id="pro_add_time" addDate="<?php echo $row->pro_add_time?>" readonly="true"/>&nbsp;&nbsp;
							</div>
							<div class="col-xs-2">
					...
					<tr>
						...
						<td>
							<div class="col-xs-2">
								<input type="text" class="form-control input-sm" name="pro_off_time" id="pro_off_time" offDate="<?php echo $row->pro_off_time?>" readonly="true"/>&nbsp;&nbsp;
							</div>
							<div class="col-xs-2">
						...
						<td>
							...
							<select name="pro_status" class="form-control input-sm">
								<?php 
									...
									{
								?>
										<option value="<?php echo $status_row->code_key?>" <?php if ($status_row->code_key == $row->pro_status): ?>
											selected
										<?php endif ?>	><?php echo $status_row->code_name?></option>
								<?php
									}
								?>
							</select>
					...
					<tr>
						...
						<td>
							...
							<select name="pro_promote" class="form-control input-sm">
								<option value="" <?php if ( $row->pro_promote == ""): ?>
									selected
								<?php endif ?>>無</option>
								<?php 
									...
									{
								?>
										<option value="<?php echo $promote_row->code_key?>" <?php if ($promote_row->code_key == $row->pro_promote): ?>
											selected
										<?php endif ?>><?php echo $promote_row->code_name?></option>
								<?php
									}
								?>
							</select>
...
<div style='display:none'>
	<div id="choosePlanArea">
		...
		<div class="row" style="text-align:center; margin: 20px 0 0 0;">
			<button class="btn btn-sm btn-default" id="add_plan" gaids="" url="<?php echo $add_plan_url?>">新增</button>

...
class Member_manage_model extends MY_Model {
	...
	{
		$sql = @"SELECT a.*,b.code_name as member_type_name,b.code_value2 as discount FROM mod_member a left join mod_code b on  a.member_type = b.id WHERE a.member_id=?";
	...
	public function do_edit_member($member_type, $member_id, $member_account, $member_name, $member_mobile, $member_addr, $vat_num, $inv_title, $member_city)
	{
		...
		$para = array($member_type, $member_account, $member_name, $member_mobile, $member_addr, $vat_num, $inv_title, $member_city, $member_id);
		$success = $this->db->query($sql, $para);

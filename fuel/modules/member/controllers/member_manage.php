...
class Member_manage extends Fuel_base_controller {
	...
	{
		$base_url = base_url();
		$member_type = $this->input->get_post("member_type");
		...
		$success = $this->member_manage_model->do_add_member($member_type,$member_account, $member_pass, $member_name, $member_mobile, $member_city, $member_addr, $vat_num, $inv_title);
	...
	function edit($member_id)
	{
		...
		$city_result = $this->product_manage_model->get_code('city', ' AND parent_id=-1 ORDER BY code_key ASC');
		$member_type_result = $this->product_manage_model->get_code('MT', ' AND parent_id=-1 ORDER BY code_key ASC');

		$vars['view_name'] = $view_name;
		$vars['member_results'] = $member_results;
		$vars['city_result'] = $city_result;
		$vars['member_type_result'] = $member_type_result;
		$vars['back_url'] = $base_url."fuel/member/lists";
	...
	function do_edit($member_id)
	{
		...
		$success = $this->member_manage_model->do_edit_member($member_type,$member_id, $member_account, $member_name, $member_mobile, $member_addr, $vat_num, $inv_title, $member_city);
